...
def main():
    ...
    out_tracking = cv2.VideoWriter(config.get('video', 'RT_out_4'), cv2.VideoWriter_fourcc(*'mp4v'), int(cap.get(cv2.CAP_PROP_FPS)), (width, height))
    out_detect = cv2.VideoWriter('video/RT_vid_5.mp4', cv2.VideoWriter_fourcc(*'mp4v'), int(cap.get(cv2.CAP_PROP_FPS)), (width, height))
    mot_output_path = config.get('annotations', 'anno_pred_4_RT')
